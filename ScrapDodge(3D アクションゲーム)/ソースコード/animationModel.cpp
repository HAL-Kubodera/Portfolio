...
void AnimationModel::Draw(float r, float g, float b, float a)
{
	// プリミティブトポロジ設定
	Renderer::GetDeviceContext()->IASetPrimitiveTopology(
		D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
	{
		...
		material.Diffuse = XMFLOAT4(diffuse.r * r, diffuse.g * g, diffuse.b * b, opacity * a);
		material.Ambient = material.Diffuse;
		Renderer::SetMaterial(material);
...
void AnimationModel::Draw(XMFLOAT4 color)
{
	// プリミティブトポロジ設定
	Renderer::GetDeviceContext()->IASetPrimitiveTopology(
		D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
	{
		...
		material.Diffuse = XMFLOAT4(diffuse.r * color.x, diffuse.g * color.y, diffuse.b * color.z, opacity * color.w);
		material.Ambient = material.Diffuse;
		Renderer::SetMaterial(material);
...
void AnimationModel::SetModelColor(float r, float g, float b, float a)
{
	for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
	{
		...
		Renderer::GetDeviceContext()->Map(m_VertexBuffer[m], 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
