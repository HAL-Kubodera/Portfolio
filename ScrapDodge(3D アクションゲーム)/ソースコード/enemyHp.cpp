...
ID3D11ShaderResourceView* EnemyHp::m_Texture = nullptr;
ID3D11Buffer* EnemyHp::m_VertexBuffer = nullptr;
ID3D11VertexShader* EnemyHp::m_VertexShader = nullptr;
ID3D11PixelShader* EnemyHp::m_PixelShader = nullptr;
ID3D11InputLayout* EnemyHp::m_VertexLayout = nullptr;
bool EnemyHp::m_IsLoad = false;
...
void EnemyHp::Init()
{
	...
	m_Enemy = Manager::GetScene()->GetGameObject<Enemy>();

	m_Position = HP_BAR_POS;
	m_Scale = HP_BAR_SIZE;

	m_MaxHp = (float)m_Enemy->f_MaxHp;
	m_Hp = m_MaxHp;
	m_DispHp = m_MaxHp;
	...
	pText->SetGap(TEXT_SIZE, TEXT_SIZE,0);
	pText->SetWard("ENEMY[HP");
}
