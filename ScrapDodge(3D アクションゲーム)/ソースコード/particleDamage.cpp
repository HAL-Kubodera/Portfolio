...
#include "renderer.h"
#include "polygon.h"
#include "particleDamage.h"
#include "manager.h"
#include "scene.h"
#include "camera.h"
#include "texture.h"

void ParticleDamage::Init()
{
...
void ParticleDamage::Update()
{
	...
	for (int i = 0; i < PARTICLE_MAX; i++)
	{
		if (m_Particle[i].Enable == false)
		{
			...
			m_Particle[i].Velocity.x = (rand() % PARTICLE_RANGE - PARTICLE_RANGE / 2) / REDUCE_FACTOR;
			m_Particle[i].Velocity.y = (rand() % PARTICLE_RANGE + PARTICLE_RANGE / 2) / REDUCE_FACTOR;
			m_Particle[i].Velocity.z = (rand() % PARTICLE_RANGE - PARTICLE_RANGE / 2) / REDUCE_FACTOR;
