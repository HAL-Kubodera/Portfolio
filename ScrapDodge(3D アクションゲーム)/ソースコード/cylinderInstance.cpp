...
void CylinderInstance::Init()
{
	...
	//ストラクチャーバッファ
	{
		Vector3* position = new Vector3[CYLINDER_NUM]{};
		...
		bd.ByteWidth = sizeof(Vector3) * CYLINDER_NUM;
		...
		srvd.Buffer.NumElements = CYLINDER_NUM;
	...
	m_Scale = ParticleCylinder::PARTICLE_SCALE;
}

void CylinderInstance::Update()
{
	...
	//ストラクチャーバッファ
	{
		...
		std::vector<LaserCylinder*> cylinders = Manager::GetScene()->GetGameObjects<LaserCylinder>();

		m_CylinderNum = 0;

		for (auto cylinder : cylinders)
		{
			ParticleCylinder* pCylinder = cylinder->GetParticleCylinder();
			if (pCylinder == nullptr) continue;

			ParticleCylinder::PARTICLE* particle = pCylinder->GetParticle();

			for (int i = 0; i < pCylinder->PARTICLE_MAX; i++)
			{
				if (m_CylinderNum >= CYLINDER_NUM) break;

				if (particle[i].Enable == true)
				{
					position[m_CylinderNum] = { 0.0f,0.0f,0.0f };

					position[m_CylinderNum].x = particle[i].Position.x;
					position[m_CylinderNum].z = particle[i].Position.z;
					position[m_CylinderNum].y = particle[i].Position.y;
		...
		memcpy(pos, position, sizeof(Vector3) * CYLINDER_NUM);
...
void CylinderInstance::Draw()
{
	...
	//ポリゴン描画
	Renderer::GetDeviceContext()->DrawInstanced(VER_NUM, m_CylinderNum, 0,0);

	Renderer::SetDepthEnable(true);
}

void CylinderInstance::Uninit()
{
