...
ID3D11RenderTargetView* Renderer::g_PERenderTargetView;
ID3D11ShaderResourceView* Renderer::g_PEShaderResourceView;
...
void Renderer::Init()
{
	...
	m_Device->CreateRenderTargetView(ppTexture, &rtvd, &g_PERenderTargetView);
	...
	m_Device->CreateShaderResourceView(ppTexture, &srvd, &g_PEShaderResourceView);
}
...
void Renderer::Begin()
{
	// 背景色でクリアする前に、まず RTV と DSV をセット
	m_DeviceContext->OMSetRenderTargets(1, &m_RenderTargetView, m_DepthStencilView);
	//背景を設定
	float clearColor[4] = { 0.3f, 0.1f, 0.2f, 1.0f };
...
void Renderer::BeginPE()
{
	...
	//レンダーターゲットとzバッファのセット
	m_DeviceContext->OMSetRenderTargets(1, &g_PERenderTargetView, m_DepthStencilView);

	float ClearColor[4] = { 0.0f,0.5f,0.0f,1.0f };
	m_DeviceContext->ClearRenderTargetView(g_PERenderTargetView, ClearColor);
...
ID3D11ShaderResourceView* Renderer::GetPETexture()
{
