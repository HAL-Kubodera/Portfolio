...
bool Keyboard_IsAnyKeyDown()
{
    const unsigned int* p = (const unsigned int*)&gState;

    for (int i = 0; i < 8; ++i)   // 256 / 32 = 8
    {
        if (p[i] != 0)
        {
            return true;
...
bool Keyboard_IsAnyKeyTrigger()
{
    const unsigned int* p1 = (const unsigned int*)&gState;
    const unsigned int* p2 = (const unsigned int*)&gStateOld;
